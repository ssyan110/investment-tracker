...
export const AddAssetForm: React.FC<AddAssetFormProps> = ({ type, onSave, onCancel }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (symbol && name) onSave(symbol, name, currency, AccountingMethod.AVERAGE_COST);
  };
      ...
      <div className="w-full max-w-md glass-elevated overflow-hidden animate-slide-up m-4" style={{ borderRadius: 'var(--radius-xl)' }}>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
            ...
            <input type="text" value={symbol} onChange={e => setSymbol(e.target.value.toUpperCase())} className="glass-input font-mono uppercase" placeholder={type === AssetType.CRYPTO ? 'BTC' : 'AAPL'} autoFocus required />
          </div>
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} className="glass-input" placeholder="e.g. Bitcoin, Apple Inc." required />
          </div>
          <div>
            ...
            <select value={currency} onChange={e => setCurrency(e.target.value)} className="glass-input appearance-none cursor-pointer">
              <option value="TWD">TWD</option><option value="USD">USD</option><option value="USDT">USDT</option><option value="JPY">JPY</option><option value="EUR">EUR</option>
            </select>
          </div>
          <p className="text-[10px] ml-1" style={{ color: 'var(--text-quaternary)' }}>Accounting method: Average Cost</p>
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onCancel} className="glass-btn flex-1">Cancel</button>
            <button type="submit" className="glass-btn glass-btn-primary flex-1">Create</button>
          </div>
        </form>
      </div>
    </div>
